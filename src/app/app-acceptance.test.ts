...
describe("Toolcraft template app acceptance coverage", () => {
  ...
  it("defaults generated apps to new Toolcraft assembly mode", () => {
    ...
    if (appTransferMode.animationIntent?.mode !== "timeline-playback") {
      throw new Error("Expected a timeline-playback animation intent.");
    }
    expect(appTransferMode.animationIntent.loopDuration).toMatchObject({
      seconds: 11.88166666666667,
      source: "reference",
    });
    expect(appTransferMode.videoReferenceStudy?.storyboard.length).toBeGreaterThan(0);
  });
