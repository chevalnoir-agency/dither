...
export async function dragToolcraftSliderToValue(
  ...
  const field = await getToolcraftFieldByLabel(page, label);
  const slider = field.locator('[data-slot="slider"], [role="slider"]').first();
  const ariaSlider = field.locator('[role="slider"]').first();
  const valueElement = (await ariaSlider.count()) > 0
    ? ariaSlider
    : field.locator('[data-slot="slider"]').first();

  await expect(slider, `Toolcraft slider "${label}" should be visible`).toBeVisible();
  await expect(
    valueElement,
    `Toolcraft slider "${label}" should expose range metadata.`,
  ).toBeVisible();

  const range = await valueElement.evaluate((element) => {
  ...
  const editValueButton = field.getByRole("button", {
    name: `Edit ${label} value`,
  }).first();

  if (await editValueButton.isVisible().catch(() => false)) {
    await editValueButton.click();
    const editor = page.getByRole("textbox", { name: `${label} value` }).first();
    await expect(editor, `Editable value for "${label}" should be visible`).toBeVisible();
    await editor.fill(String(value));
    await editor.press("Enter");
    await expect(editValueButton).toContainText(String(value), { timeout: 2000 });
